...
async function googleCallback(req: Request, env: Env): Promise<Response> {
  ...
  if (!tokenRes.ok) return json({ ok: false, error: tokens.error || "token failed" }, { status: 400 });
  ...
  if (!profileRes.ok) return json({ ok: false, error: "profile failed" }, { status: 400 });
  ...
  if (deviceCode) {
    await env.KV.put(`cli_done:${deviceCode}`, JSON.stringify({
      sessionId: session.id,
      email: session.email,
      token: sealed,
    }), { expirationTtl: 300 });
    await env.KV.delete(`cli_state:${state}`);
    return Response.redirect(`${env.SITE_URL}/app?cli=connected`, 302);
  }
